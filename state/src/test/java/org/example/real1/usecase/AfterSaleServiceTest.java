...
class AfterSaleServiceTest {
	...
	void givenNewOrder_whenOrder_thenOrderCreated() {
		Repository.FLOW_MAP.clear();
		Repository.ORDER_MAP.clear();
	...
	void givenNewOrder_whenKFHandleFinished_thenOrderFinished() {
		Repository.FLOW_MAP.clear();
		Repository.ORDER_MAP.clear();
		Operator operator = new Operator(13155, "Allen");
		// Step1. 工单新建
		AfterSaleUseCase.CreateOrderCommand command = new AfterSaleUseCase.CreateOrderCommand(operator);
		...
		Assertions.assertEquals(1, Repository.FLOW_MAP.size());
		// Step2. 客服领取
		operator = new Operator(13155, "Allen2");
	...
	@Test
	void givenNewOrder_whenFinancialReject_thenOrderHandling() {
		Repository.FLOW_MAP.clear();
		Repository.ORDER_MAP.clear();
		Operator operator = new Operator(13155, "Allen");
		// Step1. 工单新建
		AfterSaleUseCase.CreateOrderCommand command = new AfterSaleUseCase.CreateOrderCommand(
			operator,
			new OrderRefundCost.RefundCash(new BigDecimal(1)),
			new OrderRefundCost.RefundCash(new BigDecimal(0))
		);
		Order.OrderId orderId = afterSaleServiceUnderTest.create(command);
		Assertions.assertNotNull(orderId);
		Assertions.assertEquals(1, Repository.ORDER_MAP.size());
		...
		afterSaleServiceUnderTest.submit(submitCommand);
		Assertions.assertEquals(1, Repository.ORDER_MAP.size());
		Assertions.assertEquals(StatePersistenceMapping.FINANCIAL_HANDLING.getCode().value(), Repository.ORDER_MAP.get(orderId.value()).getState());
		Assertions.assertEquals(3, Repository.FLOW_MAP.size());
		// Step4. 财务退回
		AfterSaleUseCase.FinancialOrderCommand financialOrderCommand = new AfterSaleUseCase.FinancialOrderCommand(orderId, operator);
		afterSaleServiceUnderTest.financialReject(financialOrderCommand);
		Assertions.assertEquals(1, Repository.ORDER_MAP.size());
		Assertions.assertEquals(StatePersistenceMapping.HANDLING.getCode().value(), Repository.ORDER_MAP.get(orderId.value()).getState());
		Assertions.assertEquals(4, Repository.FLOW_MAP.size());
		Repository.FLOW_MAP.forEach(System.out::println);
		Repository.ORDER_MAP.forEach(System.out::println);
	}
}
